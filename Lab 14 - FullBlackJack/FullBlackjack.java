...
public class FullBlackjack
{
    public static void main(String[] args){
        ...
        Random rand = new Random(); //upperbound is 10 so it will choose numbers from 0-9 when assigning varibles a random number, I'll add a +2 to make it from 2 to 11 (shifting all random values up by 2)
        ...
        while (playAgain){
            String whoBusted;
            String whoWon;//this is so I can use less print statements at the end because only 2 words are changing
            int playerTotal;
            int dealerTotal;
            int hitOrStandNum;
            int num1 = rand.nextInt(10) + 2;//like said up above, to make it from 2 to 11, I add to to "shift" all the values up by two
            int num2 = rand.nextInt(10) + 2;//num1 and num2 will be used to store the card numbers for later use
            boolean hitOrStand;
            boolean somebodyBusted = false;
            boolean tieOrBothBust;
            playerTotal = num1+num2;
            while (playerTotal >=21){//this secretly changes the numbers back into good numbers if they're bad without anyone knowing until good numbers are drawn
            ...
            while (hitOrStandNum > 2 || hitOrStandNum <= 0){//if any trolls think they can get by by typing a number other than 1 or two, this safeguard is here to protect
                out.print("Would you like to (1)Hit or (2)Stand? ");
            }
